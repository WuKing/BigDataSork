...
public class MergeSort
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		Runtime.getRuntime().exec(
				new String[] { "cmd", "/c", "del", "c:\\test\\temp\\*.txt" });
	}
			...
			String outputPath) throws Exception
	{
		...
		if (fileInfoSet.isEmpty())
		{
			for (int i = 0; i < splitFilePathList.size(); i++)
			{
				...
				fileInfo.setFileNum(Integer.parseInt(splitFilePath.substring(
						splitFilePath.lastIndexOf("\\") + 1,
						splitFilePath.indexOf(".txt"))));// 文件号
