...
public class LoserTree
{
	...
	/**
	 * all leaf nodes indexes[0] keep the global winner's index
	 */
	...
	/**
	 * hold zero position for global winner and leaf start from nodes.length/2
	 */
	private void init(RecordStore[] stores) throws IOException
	{
		...
		/**
		 * lowExt= total - (2*s-1) and total=nLeaf+(nLeaf-1) so,
		 * lowExt=2*(nLeaf-s)
		 */
		lowExt = 2 * (nLeaf - s);
	...
	private void play(int parentIndex, int leftChild, int rightChild)
	{
		...
		if (leftChild > 0 && rightChild > 0
				&& nodes[leftChild - 1].compareTo(nodes[rightChild - 1]) < 0)
		{
			winner = leftChild;
			loser = rightChild;
		}
		...
		while (parentIndex > 0 && (parentIndex % 2 == 1))
		{
			...
			if (indexes[parentIndex] != 0
					&& nodes[indexes[parentIndex] - 1]
							.compareTo(nodes[winner - 1]) < 0)
			{
				loser = winner;
